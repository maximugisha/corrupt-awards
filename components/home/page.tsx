...
import {HNominee, HStatistics, HInstitution, HRating} from '@/types/interfaces';

export default function Home() {
  const [topNominees, setTopNominees] = useState<HNominee[]>([]);
  const [topInstitutions, setTopInstitutions] = useState<HInstitution[]>([]);
  const [statistics, setStatistics] = useState<HStatistics | null>(null);
  const [isLoading, setIsLoading] = useState(true);
  const [searchQuery, setSearchQuery] = useState('');
  const [searchResults, setSearchResults] = useState<{
    nominees: HNominee[];
    institutions: HInstitution[];
  ...
  const calculateAverageRating = (ratings: HRating[]) => {
